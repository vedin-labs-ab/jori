...
const generatedPath = path.join(
  root,
  "convex/apps/templates/_generated/templates.ts"
)
...
async function compileAppTemplates(checkMode: boolean) {
  ...
  try {
    ...
    await writeFiles(template, runtimeAssets.app.template)
    ...
    const briefing = await buildApp([
      ...
      },
      {
        ...
        content: `export const appRuntimeRevision = "${runtimeRevision()}"\n`,
      },
    ])
...
async function requireCurrent(filePath: string, content: string) {
  ...
  if (existing !== content) {
    throw new Error(
      "App templates are stale. Run `pnpm templates:compile` and commit the result."
    )
  }
}
 ...
 *  plus the app template directory when the playbook has one. Both
...
function runtimeRevision() {
  return createHash("sha256")
    .update(JSON.stringify(runtimeAssets.app.template))
    .digest("hex")
}
...
function renderGenerated(value: unknown) {
  return [
    "/* biome-ignore-all lint/suspicious/noTemplateCurlyInString: generated app source */\n",
 ...
 *  not-yet-provisioned app without the server-only template payload. */
...
function format(filePath: string, content: string) {
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr || "Could not format app templates.")
  }

  return result.stdout
}

await compileAppTemplates(process.argv.includes("--check"))
