...
test("provider settings come from the selected deployment through a private pipe", () => {
  output("  project-key\n")
  expect(requireDeploymentVariable("BL_API_KEY", env)).toBe("project-key")
  expect(spawnSync).toHaveBeenCalledWith(
    expect.any(String),
    ["convex", "env", "get", "BL_API_KEY"],
...
test("missing settings never fall back to a local provider account", () => {
  output("")
  expect(deploymentVariable("BL_WORKSPACE", env)).toBeUndefined()
  expect(() => requireDeploymentVariable("BL_API_KEY", env)).toThrow(
    "Missing BL_API_KEY"
  )
})
...
test("failed reads omit provider output and require an explicit target", () => {
  output("secret", 1)
  expect(() => deploymentVariable("BL_API_KEY", env)).toThrow(
    "Could not read BL_API_KEY from the selected deployment"
  )
  expect(() => deploymentVariable("BL_API_KEY", {})).toThrow(
