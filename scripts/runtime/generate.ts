...
function generateRuntimeAssets({ checkMode }: GenerateOptions) {
...
function readBuilderFiles() {
  const builderRoot = path.join(sourceRoot, "builder")
  const files: Record<string, string> = {}
    ...
    files[outputPath] =
      sourcePath === "index.ts"
        ? rewriteBuilderEntrypointImports(
            transpileTypeScript(content, `builder/${sourcePath}`)
          )
...
function renderGeneratedModule(runtimeAssets: RuntimeAssets) {
  return [
    "// Generated by scripts/runtime/generate.ts. Do not edit by hand.",
    "\n",
