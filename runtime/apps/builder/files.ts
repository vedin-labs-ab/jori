...
export async function copyPlatformTemplate(project: string) {
  const templateFiles = await listFiles(config.appTemplatePath)

  for (const filePath of templateFiles) {
    const relativePath = normalizeAssetPath(
      path.relative(config.appTemplatePath, filePath)
    )
...
export async function readWorkspaceSource(
...
): Promise<AppSourceFile[]> {
  const stats = await fs.stat(workspacePath).catch(() => null)

  if (stats === null || !stats.isDirectory()) {
    throw new Error(`App workspace does not exist: ${workspacePath}`)
  }
  ...
  const files = (await listFiles(path.join(workspacePath, "src")))
    ...
    .filter((filePath) => !isPlatformAppSourcePath(filePath))
...
export function toPublishSource(files: NormalizedAppSourceFile[]) {
...
async function readSourceFile(project: string, file: NormalizedAppSourceFile) {
  const content = await fs.readFile(path.join(project, file.path), "utf8")
  const bytes = Buffer.from(content, "utf8")

  if (bytes.byteLength === 0) {
    throw new Error(`App source file ${file.path} is empty after formatting.`)
  }

  if (bytes.byteLength > maxAppFileBytes) {
    throw new Error(
      `App source file ${file.path} exceeds ${maxAppFileBytes} bytes.`
    )
  }
