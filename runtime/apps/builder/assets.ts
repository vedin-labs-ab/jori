...
function createRuntimeManifest(assetPaths: string[]): AppManifest {
  const entry = assetPaths.find(
    (assetPath) => assetPath.startsWith("assets/") && assetPath.endsWith(".js")
  )

  if (entry === undefined) {
    throw new Error("App build did not produce a JavaScript entry asset.")
  }
