...
export async function formatAppSource(project: string, sourcePaths: string[]) {
...
export async function runAppChecks(project: string, sourcePaths: string[]) {
...
async function writeTypeScriptConfig(project: string, sourcePaths: string[]) {
  const configPath = path.join(project, "tsconfig.app.json")
...
async function runCommand(
  ...
  try {
    ...
    const suffix = output === "" ? "." : `:\n${output}`
    throw new Error(`App ${options.label} check failed${suffix}`)
  }
}
