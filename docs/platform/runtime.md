...
E2B sandbox identity is stored directly on executions. There is no sandbox table. The Convex backend orchestrates the sandbox and execution lifecycle. Codex runs in E2B with internet access for browsing and research, but the sandbox is treated as untrusted and does not receive provider credentials.

Executions use the `milo-codex` E2B sandbox template unless `E2B_SANDBOX_TEMPLATE` overrides it. The template pre-installs Node.js, git, `@openai/codex@0.139.0`, and the MCP SDK so each message only needs Codex auth, broker config, and scoped sandbox files.

Codex authentication is stored as a Convex environment variable and copied into the ephemeral E2B sandbox as `auth.json` at runtime. Provider tools are exposed to Codex through Milo-owned MCP adapters in E2B. The tools remain provider-namespaced, such as Slack, GitHub, Linear, Gmail, Google Calendar, Notion, Microsoft Email, and Microsoft Calendar, but every provider tool call is forwarded to Milo's Convex MCP broker with the execution token. The broker validates the token hash against the active execution, derives `tenantId`, refreshes and loads provider credentials server-side, checks runtime tool permissions, and calls provider APIs from Convex. Each sandbox only receives `MILO_EXECUTION_TOKEN`, `MILO_CONVEX_SITE_URL`, enabled tool names, and non-secret adapter files.

Slack exposes context and reply tools scoped to the triggering channel. Linear exposes issue read/comment tools scoped to the triggering issue. GitHub exposes repository context, tarball-backed repository materialization, and comment replies scoped to the triggering issue or pull request. Google and Microsoft expose user-scoped mail and calendar tools when those integrations are active for the resolved execution user. Notion exposes tenant-scoped workspace context, page, record, block, and comment tools for the pages and databases selected during authorization. Milo scheduling tools use the same broker path and derive `tenantId` from the execution token.
