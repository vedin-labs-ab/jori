...
A Slack message creates a message record. If the message asks Milo to work, Milo creates a trigger, starts an execution caused by that trigger, activates the conversation, runs Codex in an E2B sandbox, stores the raw Codex agent trace as a Convex file, and lets Codex reply through Slack MCP.
