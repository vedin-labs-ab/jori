...
function SidebarProvider({
  ...
  /** Off for a sidebar that is one of several on a page: the shortcut
   *  listens on the window, so each would answer it, and take the key
   *  from the browser. */
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          // Floating pads all round for its box. Inset has no box to
          // clear, so it pads only to meet the inset's top and bottom and
          // keeps the icon rail as narrow as the plain sidebar's.
          variant === "floating" &&
...
/** `as` lets an embedded console be a section: a document has one main. */
function SidebarInset({
  as: Inset = "main",
  className,
  ...props
}: React.ComponentProps<"main"> & { as?: "main" | "section" }) {
  return (
    <Inset
      data-slot="sidebar-inset"
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[collapsible=offcanvas]:ml-2",
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        // pointer-events-none when icon-collapsed: the label collapses via
        // -mt-8 + opacity-0, which leaves an invisible box OVER the buttons
        // beneath it that would otherwise swallow their clicks.
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:pointer-events-none group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        showOnHover &&
          "group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 peer-data-active/menu-button:text-sidebar-accent-foreground aria-expanded:opacity-100 pointer-fine:opacity-0",
        className
      )}
      {...props}
    />
