...
test("renders sigil-free pills whose icons carry the kind", async () => {
  ...
  const skill = field.container.querySelector(
    '[data-job-reference-kind="skill"]'
  )
  const tool = field.container.querySelector('[data-job-reference-kind="tool"]')
  ...
  expect(tool?.getAttribute("data-job-reference-access")).toBe("unresolved")
...
test("reacts when personal integration and tool references become organization-scoped", async () => {
  ...
  expect(readyTool?.getAttribute("data-job-reference-access")).toBe("ready")
  ...
  await waitFor(() => {
    expect(
      field.container
        .querySelector('[data-job-reference-kind="tool"]')
        ?.getAttribute("data-job-reference-scope")
    ).toBe("blocked")
  })

  const integration = field.container.querySelector("[data-job-surface-scope]")
  const tool = field.container.querySelector('[data-job-reference-kind="tool"]')

  expect(integration?.getAttribute("data-job-surface-scope")).toBe("blocked")
