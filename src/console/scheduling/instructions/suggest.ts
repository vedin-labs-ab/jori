...
export type InstructionSuggestionState = {
  ...
  style: CSSProperties
  suggestions: ScheduleSurfaceSuggestion[]
}
...
export function getInstructionSuggestionState(
  ...
  const activeMention = findActiveScheduleSurfaceMention(
    textBeforeCursor,
  ...
  const suggestions = getScheduleSurfaceSuggestions(activeMention.query)
