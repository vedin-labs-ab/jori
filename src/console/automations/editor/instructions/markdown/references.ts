...
type HydrationContext = {
  catalog: AutomationMentionCatalog
  permissions?: AutomationPolicyPermissions
  tools: Map<AutomationSurfaceIntegration, string[]>
}
...
function hydrateText(
  ...
  const mentions = readAutomationMentions(scanText, context.catalog).filter(
...
function mentionNode(
  ...
  const tools =
    ...
    getDefaultAutomationSurfaceTools(integration, context.permissions)
...
export function readInstructionSurfaces(document: JSONContent) {
  ...
  visitDocument(document, (node) => {
    ...
    if (node.type !== automationSurfaceNodeName || integration === null) {
      return
    }
...
export function readInstructionReferences(document: JSONContent) {
  ...
  visitDocument(document, (node) => {
    ...
    if (
      node.type === automationReferenceNodeName &&
      kind !== null &&
      typeof id === "string"
    ) {
