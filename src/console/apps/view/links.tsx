...
type ShareLink = NonNullable<
  ReturnType<
    typeof usePaginatedQuery<typeof api.apps.console.pageShares>
  >["results"]
>[number]
    ...
    <Dialog onOpenChange={setOpen} open={open}>
      <div className="group/action absolute top-12 left-0 z-30 w-28">
        <DialogTrigger asChild>
          ...
          >
            <Link2 />
            <AppRailLabel>Links</AppRailLabel>
          </Button>
        </DialogTrigger>
      </div>
      <AppLinksDialog
...
function AppLinksDialog({
  ...
  const shares = usePaginatedQuery(
    api.apps.console.pageShares,
    open ? { appId, organizationId } : "skip",
    ...
    <DialogContent bodyClassName="gap-0 p-0" className="sm:max-w-md">
      <DialogHeader className="p-4 pr-10">
        ...
        <DialogDescription className="sr-only">
          Manage active and expired share links for this app.
        </DialogDescription>
      </DialogHeader>
      <ShareLinksContent
        appId={appId}
