...
const contributors: Contributor[] = [
  contributor("triage", "event", 49_300_000, 652, 2),
  contributor("flaky", "event", 31_200_000, 300, 3),
  contributor("changelog", "event", 22_450_000, 26, 0),
  contributor("digest", "weekdays", 22_150_000, 22, 1),
  contributor("competitor", "daily", 18_600_000, 30, 0),
  contributor("watch", "daily", 18_350_000, 30, 1),
  {
    key: "interactive",
    cadence: "event",
    micros: 14_900_000,
  ...
  {
    ...contributor("release", "weekly", 9_400_000, 4, 0),
    weekday: 5,
  },
  {
    ...contributor("chase", "weekly", 400_000, 5, 0),
    weekday: 1,
  },
]

export function demoUsage(now: number, jobs: Job[]): UsageRow[] {
  const days = Array.from(
    { length: historyDays },
    (_, offset) => now - offset * day
  )

  const rows = contributors.flatMap((entry) => {
    const job = jobs.find((job) => job.id === jobId(entry.key))
    if (entry.key !== "interactive" && job === undefined) {
      throw new Error(`Unknown usage contributor: ${entry.key}`)
    }
    return contributorRows(entry, days, job)
  })
...
function contributor(
  ...
  return { key, cadence, micros, ended, failed }
}
...
function contributorRows(
  ...
  const identity = {
    ...
    ...(job === undefined ? {} : { job: { id: job.id, label: job.name } }),
  }
