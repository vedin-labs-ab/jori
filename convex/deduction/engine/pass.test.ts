...
test.each([
  false,
  true,
])("a queued or forced pass makes no model or context calls, force=%s", async (force) => {
...
test.each([
  ...
  ["workstream", "window"],
  ["workstream", "full"],
] as const)("the paused opener cannot restart %s/%s or change history", async (stage, scope) => {
