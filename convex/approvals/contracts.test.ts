...
test.each([
  ...
  "approval cancellation normalizes %s to its documented %s result",
  async (storedStatus, expectedStatus) => {
...
test.each(["decided", "expired", "failed"])(
  "cancellation response does not expose internal %s status",
  (status) => {
    expect(
      schemaViolations(
