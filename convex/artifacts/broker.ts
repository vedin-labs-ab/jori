...
async function callAuthorizedExternalTool(
  ...
  await ctx.runMutation(internal.artifacts.cache.invalidate, {
    tenantId: context.tenantId,
...
async function callCachedArtifactTool(
  ...
  if (!input.cache.forceRefresh) {
    const cached = await ctx.runMutation(internal.artifacts.cache.read, {
      tenantId: context.tenantId,
      artifactId: context.artifactId,
      versionId: context.versionId,
      userId: context.userId,
  ...
  if (canStoreArtifactToolCacheValue(value)) {
    await ctx.runMutation(internal.artifacts.cache.write, {
      tenantId: context.tenantId,
      artifactId: context.artifactId,
      versionId: context.versionId,
      userId: context.userId,
