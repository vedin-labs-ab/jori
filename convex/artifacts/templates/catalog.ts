...
/**
 * A playbook's artifact template: compiled source, prebuilt assets, and
 * contract from the in-repo template, joined with the catalog metadata that
 * versions it. Instantiating one is an ordinary artifact publish — the
 * template only decides the content.
 */
export type ArtifactTemplate = PlaybookArtifact & {
  key: string
  version: number
  access: Scope
...
export function readArtifactTemplate(
  key: string
  ...
  if (definition.artifact === undefined) {
    throw new Error(`Playbook ${key} has a template but no artifact metadata.`)
  }
