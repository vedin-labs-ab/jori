...
import { hashArtifactSource, type NormalizedArtifactSourceFile } from "./source"

export type ArtifactBuildInput = {
  sourceHash: string
  assets: ArtifactBuildAssetInput[]
}

type ArtifactBuildAssetInput = {
  path: string
  mimeType: string
...
export function validateArtifactBuild(
  files: NormalizedArtifactSourceFile[],
  build: ArtifactBuildInput
) {
  const sourceHash = hashArtifactSource(files)

  if (build.sourceHash !== sourceHash) {
    throw new Error("Artifact build was not produced from this source.")
  }

  if (build.assets.length === 0) {
    throw new Error("Artifact build must include assets.")
  }

  if (build.assets.length > maxBuildAssets) {
    throw new Error(
      `Artifact build can include at most ${maxBuildAssets} assets.`
    )
  }
  ...
  const assets = build.assets.map((asset) => {
    ...
    if (paths.has(assetPath)) {
      throw new Error(`Duplicate artifact build asset path: ${assetPath}`)
    }
    ...
    if (bytes.byteLength === 0) {
      throw new Error(`Artifact build asset ${assetPath} is empty.`)
    }

    if (bytes.byteLength > maxBuildAssetBytes) {
      throw new Error(
        `Artifact build asset ${assetPath} exceeds ${maxBuildAssetBytes} bytes.`
      )
    }
  ...
  if (totalBytes > maxBuildBytes) {
    throw new Error(`Artifact build exceeds ${maxBuildBytes} total bytes.`)
  }
...
function validateManifest(assets: BuildAsset[]) {
  ...
  if (manifest === undefined) {
    throw new Error("Artifact build is missing milo-manifest.json.")
  }
  ...
  if (typeof parsed.entry !== "string") {
    throw new Error("Artifact build manifest is missing an entry asset.")
  }

  const assetPaths = new Set(assets.map((asset) => asset.path))

  if (!assetPaths.has(parsed.entry)) {
    throw new Error("Artifact build manifest entry asset is missing.")
  }

  if (!parsed.entry.startsWith("assets/") || !parsed.entry.endsWith(".js")) {
    throw new Error("Artifact build manifest entry must be a JavaScript asset.")
  }
  ...
  if (
    ...
    throw new Error("Artifact build manifest styles are invalid.")
  }
}
...
function normalizeAssetPath(assetPath: string) {
  ...
  if (normalized === "") {
    throw new Error("Artifact build asset paths cannot be empty.")
  }

  if (normalized.startsWith("/") || normalized.endsWith("/")) {
    throw new Error(`Artifact build asset path must be relative: ${assetPath}`)
  }
  ...
  if (
    ...
    throw new Error(`Artifact build asset path is not allowed: ${assetPath}`)
  }
