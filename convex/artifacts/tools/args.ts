...
export function normalizeBuild(value: unknown) {
  if (!isRecord(value)) {
    throw new Error(
      "Artifact source must be validated by the Milo artifact builder before publishing."
    )
  }

  if (!Array.isArray(value.assets)) {
    throw new Error("Artifact build assets are required.")
  }
...
function normalizeBuildAsset(asset: unknown) {
  if (!isRecord(asset)) {
    throw new Error("Artifact build assets must be objects.")
  }
