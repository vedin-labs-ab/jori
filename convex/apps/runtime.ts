...
export const authorizeTool = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    if (permission === undefined) {
      throw new Error(`Unknown app tool: ${args.tool}`)
    }
    ...
    if (capability === null) {
      throw new Error(`App is not allowed to use tool: ${args.tool}`)
    }
...
async function findActiveCapability(
  ...
  const capabilities = await ctx.db
    .query("appTools")
    .withIndex("by_app_and_tool", (index) =>
      index.eq("appId", args.appId).eq("tool", args.tool)
    )
    .take(50)
...
function matchesIntegration(
  capability: Pick<Doc<"appTools">, "integrationId">,
