...
test("summarizes app capabilities by canonical tool surface", () => {
  expect(
...
test("preserves app capability integration scope metadata", () => {
  const integrationId = "integration" as Id<"integrations">
  const versionId = "version" as Id<"appVersions">
...
test("omits unknown app capabilities from console summaries", () => {
  expect(
    summarizeCapabilityForConsole({
      approvedAt: 1,
      integrationId: undefined,
