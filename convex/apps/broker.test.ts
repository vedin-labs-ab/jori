...
describe("app broker cache", () => {
  test("caches external read tool results by exact request", async () => {
    ...
    const first = await callAppTool(fixture.ctx, platformContext(), {
      args: { channel: "C123", ts: "123.456" },
      tool: "conversations_replies",
    })
    const second = await callAppTool(fixture.ctx, platformContext(), {
  ...
  test("forceRefresh bypasses a matching cache entry", async () => {
    ...
    await callAppTool(fixture.ctx, platformContext(), {
      args: { channel: "C123", ts: "123.456" },
      tool: "conversations_replies",
    })
    const refreshed = await callAppTool(fixture.ctx, platformContext(), {
      args: { channel: "C123", ts: "123.456" },
      forceRefresh: true,
      tool: "conversations_replies",
    })
