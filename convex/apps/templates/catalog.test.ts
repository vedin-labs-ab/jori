...
import { readAppTemplate, templatePartition } from "./catalog"

const template = readAppTemplate("meeting-briefing")

describe("app template catalog", () => {
  test("reads known templates only", () => {
    expect(template).toBeDefined()
    expect(readAppTemplate("missing")).toBeUndefined()
  })
  ...
  test("joins catalog metadata with the compiled template", () => {
    ...
    expect(template?.title).toBe(definition.app?.title)
    expect(template?.description).toBe(definition.app?.description)
    expect(template?.access).toBe(definition.scope)
  })

  test("client contract summary mirrors the compiled contract", () => {
    const compiled = normalizeAppContract(template?.contract)
...
function assertBriefingState(state: unknown) {
  ...
  const entry = resolveAppStateContract(
    normalizeAppContract(template.contract),
    "briefings"
  )
