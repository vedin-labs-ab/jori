...
test.each([
  "eu",
  "us",
])("%s install uses its client and callback, with organizational accounts", async (region) => {
...
test.each([
  "eu",
  "us",
])("%s callback handoff stays on its own frontend", async (region) => {
...
])("does not send a bearer token to an invalid pagination URL: %s", async (nextLink) => {
  const fetch = vi.fn()
