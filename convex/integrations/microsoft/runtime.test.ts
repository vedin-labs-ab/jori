...
test.each(["microsoftEmail", "microsoftCalendar"] as const)(
  "%s reuses fresh credentials without network or persistence",
  async (integration) => {
    const fetch = vi.fn()
...
test.each(["microsoftEmail", "microsoftCalendar"] as const)(
  "%s refreshes through its tenant and persists rotated tokens",
  async (integration) => {
...
test.each([
  ...
  "handles $error without persisting invalid tokens",
  async ({ error, expired }) => {
    const fetch = vi
      .fn()
      .mockResolvedValue(
