...
test.each([
  ...
  undefined,
])("repository adapters preserve description %j and satisfy their contracts", async (description) => {
  const repository = {
    id: 123,
    full_name: "acme/app",
    private: true,
  ...
  const responses = [{ total_count: 1, repositories: [repository] }, repository]
  vi.stubGlobal(
    "fetch",
...
test.each([
  ...
  [],
  {},
])("repository contracts reject invalid description %j", (description) => {
  expect(
    schemaViolations(
      { description },
