...
test.each([
  null,
  user,
])("Linear issue adapters preserve nullable users %j", async (person) => {
  const issue = {
    id: "synthetic-issue",
    identifier: "ENG-42",
  ...
  const responses = [
    ...
    {
      data: { issue: { ...issue, description: null, comments: { nodes: [] } } },
    },
  ]
  vi.stubGlobal(
    "fetch",
...
])("Linear comments preserve nullable parent and author %j", async (references) => {
  const comment = {
...
test.each([
  ...
  1,
  false,
])("Linear contracts reject non-object references %j", (value) => {
