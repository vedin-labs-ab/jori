...
test.each(["queued", "running", "completed", "failed", "stopped"] as const)(
  "loads a %s run with its retained sandbox",
  async (status) => {
...
test.each(["job", "instruction"] as const)(
  "limits %s requester accounts to the run's integration grants",
  async (type) => {
    const t = convexTest(schema, modules)
