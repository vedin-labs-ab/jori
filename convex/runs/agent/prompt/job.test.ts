...
import {
  jobRuntimeInput,
  linearJobRuntimeInput,
  notionJobRuntimeInput,
} from "../../../../test/convex/prompt"
import { assemblePrompt } from "."

describe("job trigger prompts", () => {
  test("omits empty job context", () => {
    const prompt = assemblePrompt(jobRuntimeInput())
    const context = prompt.context
    const instructions = prompt.instructions

    expect(context).toContain("This job run fired on its schedule.")
...
describe("raw Markdown trigger instructions", () => {
  test("appends job instructions as raw Markdown", () => {
...
describe("requester local time", () => {
  test("renders when the requester's timezone is known, else stays out", () => {
    ...
    expect(assemblePrompt(jobRuntimeInput()).context).not.toContain(
      "Requester local time:"
    )
  })
})

describe("job event prompts", () => {
  test("renders integration target context for Linear events", () => {
    const prompt = assemblePrompt(linearJobRuntimeInput()).context
  ...
  test("renders integration target context for Notion events", () => {
    const prompt = assemblePrompt(notionJobRuntimeInput()).context
