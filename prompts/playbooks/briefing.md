Brief me on every eligible meeting in the window so I walk in knowing why it matters now, what changed, and what to decide, ask, or do. Honest, actionable preparation beats background.
...
Keep confidential professional meetings in scope — board and investor meetings, commercial negotiations, recruiting interviews, customer escalations — written so everything stored stays presentable. Personal or acutely sensitive matters stay out, since a shareable briefing is inappropriate there: medical or therapy appointments, personal legal or financial matters, credentials or secrets, and disciplinary, termination, harassment, or formal investigation meetings. Remove a stored meeting that turns out to be one of these.
...
Protect the configured delivery targets first, before calendar discovery or research; a target may create only its keyed retry or a moved-event replacement. Create each target with personal scope, this artifact, a key containing the parent automation ID, and only the tools it needs; retry a confirmed #add_automation failure once.
...
While today's target is still ahead, create or find one automation keyed `meeting-briefing:<parent automation ID>:morning:<target UTC>`; it owns delivery. Give it these complete instructions with literal target, window, and planner-trigger UTC values:

```txt
Deliver one Morning Briefing for meetings starting after <coverage start UTC> and at or before <coverage end UTC>.

Read the `briefings` state and re-read each candidate event from @{{providers.calendar}} by its stored identifiers, refreshing stored facts; preparation is stale when an event's `contentHash` changed. Include only current, presentable, undelivered meetings — leave out cancelled, private, sensitive, underway, changed, or already-delivered ones.
...
{% if options.beforeMeeting %}For every eligible meeting, create a one-time reminder at start minus {{options.leadMinutes}} minutes when that is still ahead, keyed `meeting-briefing:<parent automation ID>:event:<meeting key>:<start UTC>`; if the meeting is already inside that window, prepare and deliver it now. Give each automation these complete instructions with literal IDs and times:
