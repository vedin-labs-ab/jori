...
Compose a skimmable brief: a short schedule overview first, then the messages that matter with one line each on why. If the calendar is empty and nothing needs attention, say so in one line instead of padding.
