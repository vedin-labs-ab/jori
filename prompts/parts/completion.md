...
{% if tools.send_reply %}
Before `finish_run`, use the appropriate surface communication tool only when user-visible communication is warranted. Otherwise call `finish_run` with an internal `reason`.
{% endif %}
