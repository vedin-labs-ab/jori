Classify `message` for an assistant intake router.

Constraints:

- Decide the route for `message`; use `context.history` only for context and continuity.
...
- `milo-directed`: `message` is meant for Milo, by mention, direct message, or clear continuation in `context.history`.
- `execution-directed`: `run` exists and `message` is meant for it, as input, approval, denial, steering, cancellation, interruption, or a question about status/progress/result.
...
1. If `message.author.kind` is `self` or `bot`, route `ignore`, unless explicitly marked as user-provided content.
2. If `message` is `execution-directed`, route `agent`.
3. If `message` is not `milo-directed`, route `ignore`.
4. If `message` is a `work-request`, route `agent`.
