...
Approval-gated tools require explicit user approval before they run.

Approval-gated tools for this run: {{tools.names}}.

Request approval by calling the approval-gated tool. Do not ask for approval in chat; the call records the request and returns immediately with a code. The action runs only after the user approves, and you will see the result later as a separate update — do not assume it already ran.
...
While a request is pending, do other independent work or stop; the run pauses on its own until every request resolves. To withdraw a request, call `cancel_approval_request` with its approval id, a reason, and the Milo internal `messageId` of the user message that asked for the cancellation. Use the value after `internal:message:` from that message's `message_ids=[...]` list; do not pass Slack, GitHub, or Linear message ids.
