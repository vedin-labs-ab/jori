...
While a request is pending, do other independent work or stop; the run pauses on its own until every request resolves. To withdraw a request, call `cancel_approval_request` with its approval id, a reason, and the Milo internal `messageId` of the requester's message that asked for the cancellation. Use the value after `internal:message:` from that message's `message_ids=[...]` list; do not pass Slack, GitHub, or Linear message ids.
